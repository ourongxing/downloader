...
module.exports = class Download {
  ...
  async start() {
    ...
    try {
      ...
      await this._handlePossibleStatusCodeError({
        dataStream,
        originalResponse
      })
      ...
      let { finalFileName, originalFileName } = await this._getFileName(
  ...
  async _createErrorObject(dataStream, originalResponse) {
    ...
    const error = new Error(
      `Request failed with status code ${originalResponse.statusCode}`
    )
    ...
    error.responseBody = isJson(responseString)
  ...
  async _shouldSkipSaving(originalFileName) {
    if (
      this.config.skipExistingFileName &&
      (await exists(this.config.directory + "/" + originalFileName))
    ) {
      return true
    }
    return false
  ...
  _getProgressStream() {
    ...
    const progress = new Transform({
      transform(chunk, encoding, callback) {
        ...
        if (that.fileSize) {
          that.percentage = (
            (that.currentDataSize / that.fileSize) *
            100
          ).toFixed(2)
        } else {
          that.percentage = NaN
  ...
  async _getFileName(responseHeaders) {
    ...
    if (this.config.cloneFiles === true) {
      var fileProcessor = new FileProcessor({
        useSynchronousMode: this.config.useSynchronousMode,
